...
import static hhp.tictactoe.learning.supervised.Trainer.train;
import static hhp.tictactoe.learning.supervised.Utility.humanInput;
import static hhp.tictactoe.learning.supervised.Utility.machineLearningInput;
import static hhp.tictactoe.learning.supervised.Utility.randomInput;
import static hhp.tictactoe.learning.supervised.Utility.turn;
			...
			for (int x = 0; x < 10; x++) {
				...
				for(int i = 0; i < Integer.MAX_VALUE; i++) {
		    		...
		    		if(timePlayed >= 3 && winYet(boardArray,1)) {
		    			if (doBackPropagation) { classifier.increaseWeight(gameData, X); }
		    		...
		    		if(timePlayed2 >= 3 && winYet(boardArray, 2)) {
		    			if (doBackPropagation) { classifier.decreaseWeight(gameData, X); }
		    			endGame("Human"); 
		    			break;
		    		}
	    ...
	    //====================================
			...
			for (int x = 0; x < 1000; x++) {
				...
				for (int i = 0; i < Integer.MAX_VALUE; i++) {
					...
					if (timePlayed >= 3 && winYet(boardArray, 1)) {
						drawBoard(boardArray);
		    			if (doBackPropagation) { classifier.increaseWeight(gameData, X); classifier2.decreaseWeight(gameData, O); }
						c1++;
						break;
					}
					if (timePlayed >= 3 && checkTie(boardArray)) { t++; drawBoard(boardArray); break; }
					//---------------
					//player 2's turn
					...
					if (timePlayed2 >= 3 && winYet(boardArray, 2)) {
						drawBoard(boardArray);
		    			if (doBackPropagation) { classifier.decreaseWeight(gameData, X); classifier2.increaseWeight(gameData, O); }
						c2++;
						break;
					}
					if (timePlayed2 >= 3 && checkTie(boardArray)) { t++; drawBoard(boardArray); break; }
				} //end for
			}
